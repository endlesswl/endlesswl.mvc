...
Util = {
	...
	ajaxPostNocheck : function (url, data1) {
		var s = "";
		$.ajax({
			type : "POST",
			url : url,
	...
	toLogin_h5 : function(ctx, returnUrl) {
		var userId = Util.getLocalUser().userid;
		var token = Util.getLocalUser().token;
		if(userId != '' && token != ''){
			window.location.href = returnUrl;
		}else{
			var url = ctx + '/location?path=lqb/login/login&redirectUrl='+returnUrl;
	...
	isMobile : function(mobile) {
		var reg0 = /^0{0,1}(13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])[0-9]{8}$/;
		var isOk = false;
		if (reg0.test(mobile)) {
			isOk=true;
		}
	...
	toAuthor:function(){
		...
		// 获取ctx
		var ctx = "";
		if (window.location.host == "wap.lingcaibao.com" || window.location.host == "betalqb.lingcaibao.com" || window.location.host == "www.tmining.com.cn") {
			ctx = "";// 线上环境
		} else {
			ctx = "/lqb";// 测试环境
		}
		var loginUrl = ctx + '/location?path=lqb/login/login&redirectUrl='+encodeURIComponent(window.location.href);
		// 重新授权
		var param = {
	...
	showErrorMessage:function(errorMessage){
		if($('#Vdialog').size()==0){
			...
			$('#Vdialog').css({'left':aLeft,'bottom':'15%'});
		}		
